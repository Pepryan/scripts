...
update_node_exporter() {
    ...
    for ((i=0; i<${#desired_version_array[@]}; i++)); do
        ...
        # if [ $current_version -lt $NEW_NODE_EXPORTER_VERSION ]; then
            ...
            if [ "$user" == "ubuntu" ]; then
                ssh -i ~/devops.pem -l "$user" "$ip" 'sudo mv /usr/local/bin/node_exporter/ /usr/local/bin/node_exporter_old_dir' 2>/dev/null
                ssh -i ~/devops.pem -l "$user" "$ip" 'sudo mv /usr/local/bin/node_exporter /usr/local/bin/node_exporter_old' 2>/dev/null
                ssh -i ~/devops.pem -l "$user" "$ip" 'sudo mv ~/node_exporter/ ~/node_exporter_old_dir' 2>/dev/null
